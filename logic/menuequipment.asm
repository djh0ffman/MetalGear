...
MenuEquipMove:
		    ...
		    ld	    a, c
		    ld	    (+vars.SelectIdx), a		    ; Set new selected index
...
ChkUseItem:
		    ...
		    ld	    a, (+vars.MaxLife)
		    ld	    (+vars.Life), a			    ; Fill the energy
...
ChkDropTransmitter:
		    ...
		    xor	    a
		    ld	    (+vars.TransmiTaken), a		    ; Erase transmitter	flag
		    jr	    UseItemSfx
...
ChkUseAntidote:
		    ...
		    xor	    a
		    ld	    (+vars.Poisoned),	a		    ; Remove poisoned status
		    jr	    UseItemSfx
...
ExitEquipMenu:
		    ...
		    ld	    (+vars.GameMode),	a		    ; Set binoculars mode

		    xor	    a
		    ld	    (+vars.BinoculStatus), a
		    inc	    a
		    ld	    (+vars.BinocularDir), a		    ; Reset binoculars mode status

		    call    DisableScreen		    ; Disable screen rendering

		    ld	    hl,	+vars.EnemyList		    ; Pointer to an array of enemies
		    ld	    de,	+vars.EnemyListCopy		    ; Pointer to an array of enemies copy
		    ld	    bc,	800h
		    ldir				    ; Backup enemies

		    ld	    a, (+vars.PowerSwitchOn)		    ; Power switch status 1=On,	0=Off/destroyed
		    ld	    (+vars.PowerSwitchOnCopy), a	    ; Save power switch	status

		    ld	    hl,	(+vars.RadioCallFlag)		    ; 1=Start incoming call, 2=Stop incoming call
		    ld	    (+vars.RadioCallFlagCopy), hl	    ; Save radio call flag and timer

		    ld	    a, (+vars.AlertModeCopy)
		    ld	    (+vars.AlertModeCopy2), a		    ; Save alert mode status
